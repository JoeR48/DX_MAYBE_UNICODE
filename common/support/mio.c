...
/* local declarations */
static TCHAR volume[VOLUMESIZ + 8];
static TCHAR member[MEMBERSIZ + 1];
 ...
 */
INT miofixname(TCHAR *nam, TCHAR *ext, INT opt)
{
	INT i1, i2, i3, i4, fioflags;
	TCHAR extension[MAX_EXTENSION_LENGTH + 1]; /* Allow for terminating null */
	UCHAR state;
	...
	extension[0] = member[0] = volume[0] = (TCHAR) '\0';
	state = 0;
	...
	for (i1 = i2; i1--; ) {
		...
		if (nam[i1] == ':') {
			...
			if ((fioflags & (FIO_FLAG_COMPATRMS | FIO_FLAG_COMPATRMSX | FIO_FLAG_COMPATRMSY)) && i3 == 5) {
				i4 = (INT)_tcslen(&nam[i1 + 1]);
				if (i4 > MAX_EXTENSION_LENGTH) i4 = MAX_EXTENSION_LENGTH;
				_tcsncpy(extension, &nam[i1 + 1], i4);
				extension[i4] = (TCHAR) '\0';
				for (i3 = 0; i3 < 4; i3++) extension[i3] = (UCHAR) toupper(extension[i3]);
				if (!_tcscmp(extension, _T("TEXT"))) _tcscpy(extension, _T(".TXT"));
				else if (!_tcscmp(extension, _T("ISAM"))) _tcscpy(extension, _T(".ISI"));
				else {
					i4 = (INT)_tcslen(&nam[i1]);
					if (i4 > MAX_EXTENSION_LENGTH) i4 = MAX_EXTENSION_LENGTH;
					_tcsncpy(extension, &nam[i1], i4 + 1);
					if (fioflags & (FIO_FLAG_COMPATRMS | FIO_FLAG_COMPATRMSX)) extension[4] = (TCHAR) '\0';
				}
			}
			else {
				i4 = (INT)_tcslen(&nam[i1]);
	...
	if (i1 < 0 && i2 > 8 && (fioflags & FIO_FLAG_COMPATRMS)) nam[8] = (TCHAR) '\0';

	if (((opt & FIXNAME_EXT_ADD) && !extension[0]) || ((opt & FIXNAME_EXT_REPLACE) && extension[0])) {
		if (!member[0]) _tcscpy(extension, ext);
		if (fioflags & FIO_FLAG_EXTCASEUPPER) for (i1 = 0; extension[i1]; i1++) extension[i1] = (TCHAR) toupper(extension[i1]);
	}
 	if ((fioflags & FIO_FLAG_NAMEBLANKSSQUEEZE) || !(fioflags & FIO_FLAG_NAMEBLANKSNOCHOP)) {
		for (i1 = (INT)_tcslen(nam); i1 && nam[i1 - 1] == ' '; i1--);
		nam[i1] = (TCHAR) '\0';
		for (i1 = (INT)_tcslen(extension); i1 && extension[i1 - 1] == ' '; i1--);
		extension[i1] = (TCHAR) '\0';
	}
	_tcscat(nam, extension);
	if ((fioflags & (FIO_FLAG_NAMECASEUPPER | FIO_FLAG_NAMECASELOWER | FIO_FLAG_PATHCASEUPPER | FIO_FLAG_PATHCASELOWER))
...
#if OS_WIN32
		for (i1 = (INT)_tcslen(nam) - 1; i1 >= 0 && nam[i1] != '\\' && nam[i1] != '/' && nam[i1] != ':'; i1--) {
			if (fioflags & FIO_FLAG_NAMECASEUPPER) nam[i1] = (TCHAR) toupper(nam[i1]);
			else if (fioflags & FIO_FLAG_NAMECASELOWER) nam[i1] = tolower(nam[i1]);
		}
#endif
#if OS_UNIX
		for (i1 = (INT)_tcslen(nam) - 1; i1 >= 0 && nam[i1] != '/' && nam[i1] != '\\'; i1--) {
			if (fioflags & FIO_FLAG_NAMECASEUPPER) nam[i1] = (TCHAR) toupper(nam[i1]);
			else if (fioflags & FIO_FLAG_NAMECASELOWER) nam[i1] = tolower(nam[i1]);
		}
#endif
		if (fioflags & (FIO_FLAG_PATHCASEUPPER | FIO_FLAG_PATHCASELOWER)) {
			for (i1--; i1 >= 0; i1--) {
				if (fioflags & FIO_FLAG_PATHCASEUPPER) nam[i1] = (TCHAR) toupper(nam[i1]);
	...
	if (volume[0] && !(opt & FIXNAME_PAR_DBCVOL)) {
		i1 = (INT)_tcslen(nam);
		nam[i1++] = ':';
		_tcscpy(&nam[i1], volume);
	}
	if (member[0] && !(opt & FIXNAME_PAR_MEMBER)) {
		i1 = (INT)_tcslen(nam);
		nam[i1++] = '(';
		_tcscpy(&nam[i1], member);
		i1 += (INT)_tcslen(member);
		nam[i1++] = ')';
	}
	return 0;
}
 ...
 * Does not move memory
 */
void miogetname(TCHAR **vol, TCHAR **mem)
{
	*vol = volume;
	*mem = member;
}
 ...
 * Does not move memory
 */
INT miogetenv(TCHAR *var, TCHAR **pptr)
{
	TCHAR *ptr;

	ptr = (TCHAR *) GETENV(var);
	if (ptr != NULL) {
		*pptr = ptr;
		return 0;
	}
 ...
 * Does not move memory
 */
INT miostrscan(TCHAR *source, TCHAR *dest)
{
	INT retflg;
...
INT miogetfilebytes(TCHAR *filename, UCHAR *memory, INT length)
{
	FHANDLE handle;
	INT count;
	TCHAR workname[MAX_NAMESIZE];

	_tcscpy(workname, filename);  /* fioaopen does slash conversion */
...
INT miogetfilechgtime(TCHAR *filename, UCHAR *timestamp)
{
#if OS_WIN32
	INT i1;
	HANDLE handle;
